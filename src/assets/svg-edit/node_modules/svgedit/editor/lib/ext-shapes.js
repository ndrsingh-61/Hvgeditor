rbhDraw.addExtension("shapes", function() {
  var current_d, cur_shape_id;
  var canv = rbhDraw.canvas;
  var cur_shape;
  var start_x, start_y;
  ...
  function loadLibrary(cat_id) {
    var lib = library[cat_id];
     if(!lib && cat_id!=='customize' && cat_id!=='basic') {
      $('#shape_buttons').html('Loading...');
      $.getJSON('lib/shapelib/' + cat_id + '.json', function(result) {
    ...
    else if(!lib && (cat_id==='customize' || cat_id==='basic')) {
      $('#shape_buttons').html('Loading...');
      $.getJSON('lib/shapelib/' + cat_id + '.json', function(result) {
        cur_lib = library[cat_id] = {
          data: result.data
  ...
  function makeButtons(cat, shapes) {
    ...
    for(var id in data) {
      ...
      if(cat==='customize'){
        icon.find('path').attr('d', data[id].path);
        icon.find('path').attr('fill', data[id].fill==='#fff'?'#000000':data[id].fill);
       //icon.find('path').attr('fill', data[id].fill);
        icon.find('path').attr('stroke', data[id].stroke);
        icon.find('path').attr('stroke-width',data[id].strokeWidth);
        icon.find('path').attr('stroke-dasharray',data[id].strokeWidth);
        }
        else if(cat==='electronics'){
          icon.find('path').attr('d', path_d);
          icon.find('path').attr('fill', '#333');
          icon.find('path').attr('stroke', 'black');
          icon.find('path').attr('stroke-width','5');
        }
        else {
          icon.find('path').attr('d', path_d);
          icon.find('path').attr('fill', '#444');
          icon.find('path').attr('stroke', 'transparent');
          icon.find('path').attr('stroke-width','5');
        }
  ...
  return {
    ...
    callback: function() {
      ...
      $('#shape_buttons').mouseup(function(evt) {
        ...
        shower.append(copy)
          .attr('data-curopt', '#' + btn[0].id) 
          .mouseup();
          
        cur_shape_id = btn[0].id.substr((mode_id+'_').length);
        if(shape_value==='customize'){
        current_d = cur_lib.data[cur_shape_id].path;
        cur_shape_name= cur_lib.data[cur_shape_id].name;
        cur_shape_fill= cur_lib.data[cur_shape_id].fill;
        cur_shape_stroke=cur_lib.data[cur_shape_id].stroke;
        cur_shape_stroke_width =cur_lib.data[cur_shape_id].strokeWidth;
        cur_shape_stroke_style=cur_lib.data[cur_shape_id].stroke_style;
        customshape_value='true';
        }
        else{
          cur_shape_name=btn[0].title;
          current_d = cur_lib.data[cur_shape_id];
          cur_shape_fill = '#333';
          cur_shape_stroke = '#000';
          cur_shape_stroke_width='1.5';
          customshape_value='false';
        }
    ...
    mouseDown: function(opts) {
      if(typeof current_d === 'object'){
        var value=current_d.path;
        cur_shape_fill=current_d.fill;
        cur_shape_stroke=current_d.stroke;
        cur_shape_stroke_width=current_d.strokeWidth;
        current_d=value;
       }
      ...
      cur_shape = canv.addSvgElementFromJson({
        ...
        "attr": {
          ...
          "customshape":customshape_value,
          "shapename":cur_shape_name,
          "fill":cur_shape_fill,
          "stroke":cur_shape_stroke,
          "stroke-width":cur_shape_stroke_width==="undefined"?'5':cur_shape_stroke_width
        }
      });
      cur_shape.setAttribute("d", current_d);
      cur_shape.setAttribute('fill', cur_shape_fill);
      cur_shape.setAttribute('stroke', cur_shape_stroke);
      cur_shape.setAttribute('stroke-width', cur_shape_stroke_width);
