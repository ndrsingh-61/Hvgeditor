...
(function() {
  ...
  //              // opts.button, default, position
      ...
      var ImportSelected= function(){
       ...
       const processFile = (node,file ) => {
        try{
          var file = file[0];
          if (file) {
              var reader = new FileReader();
              reader.onloadend = function(e) {
                loadimportSvgString(e.target.result,node);
                updateCanvas();
              };
              reader.readAsText(file, "UTF-8");
              reader.onerror = function (evt) {
...
//       $('#button_ungroup').on("click", editor.ungroupSelected);
     ...
     $('#resolution').change(function(){
      ...
      var Actions = function() {
         var tool_buttons = [
          ...
          {sel:'#tool_imports', fn: ImportSelected, evt:'click', key: [modKey+'up', true]},
    ...
    function loadimportSvgString(str, callback) {
      ...
      var groupSvg = document.createElementNS("http://www.w3.org/2000/svg", 'g');
       // mergedSvg.appendChild(groupSvg);
       // previous svg
      ...
      for (let i = 0; i < currentContent.length; i++) {
        groupSvg.appendChild(currentContent[i]);
      }
      console.log(groupSvg);
      mergedSvg.appendChild(groupSvg);
    ...
    function loadimportSvgString(str, callback,node_value) {
      ...
      const doc1 = parser.parseFromString(xmlString, "application/xml");
        console.log(doc1);
      const node = document.createElement("svg");
      for (let i=0; i<= doc1.childNodes[0].childNodes.length; i++){
        node.appendChild(doc1.childNodes[0].childnode[i]);
      }
      let value = document.getElementById('svgcontent');
      for (let j=0; j<=value.childNodes[0].childNodes.length; j++){
        node.appendChild(value.childNodes[0].childNodes[j]);
      }
