...
//              call("selected", [selected]);
      ...
      case "fhpath":
        ...
        if (keep) {
          element = path.smoothPolylineIntoPath(element);
        }
        break;
      case "line":
        var attrs = $(element).attr(["x1", "x2", "y1", "y2"]);
...
this.setSvgString = function(xmlString) {
  try {
    ...
    if (background.length) {
      ...
      rbhDraw.paintBox.canvas.setPaint(fill)
    }
    else {
...
this.setMode = function(name,svgcontent) {
   pathActions.clear();
  textActions.clear();
...
this.setRectangle_w_h = function(val,type,shapetype) {
  cur_properties.stroke_width = val;
  var elems = [];
  var i = selectedElements.length;
  while (i--) {
  ...
  if (elems.length > 0) {
    ...
    if(type==="width"){ changeSelectedAttribute("width", val, elems); }
    else if(type==="height"){ changeSelectedAttribute("height", val, elems); }
    else if(type==="x"){ changeSelectedAttribute("x", val, elems); }
    else if(type==="y"){ changeSelectedAttribute("y", val, elems); }
    else if(type==="opacity"){ changeSelectedAttribute("opacity", val, elems); }
    }
    else if(shapetype==='Path'){
      if(type==="x"){ changeSelectedAttribute("x", val, elems); }
      else if(type==="y"){ changeSelectedAttribute("y", val, elems);}
    }
    else if(shapetype==='Circle'){
      if(type==="cx"){ changeSelectedAttribute("cx", val, elems); }
      else if(type==="cy"){ changeSelectedAttribute("cy", val, elems);}
      else if(type==="r"){ changeSelectedAttribute("r", val, elems);}
    }
    else if(shapetype==='Line'){
      if(type==="x1"){ changeSelectedAttribute("x1", val, elems); }
      else if(type==="x2"){ changeSelectedAttribute("x2", val, elems);}
      else if(type==="y1"){ changeSelectedAttribute("y1", val, elems);}
      else if(type==="y2"){ changeSelectedAttribute("y2", val, elems);}
    }
...
this.deletecustomshape = function deletecustomshape(shape,path) {
  var jsonvalue={ "Path": path,"Name": shape};
        $.ajax({
          url: 'https://localhost:44311/api/employees/deletecustomshape',
          type: 'POST',
          processData: false,
...
this.readjsonfile = function readjsonfile(shape,svgcontent) {
$.getJSON('lib/shapelib/customize.json', function(result) {
  cur_lib =  {
    customshapename: result.data,
   }
   jsonvalue(cur_lib,shape,svgcontent);
   });
   return ;
}

function jsonvalue(shapesname,shape,svgcontent)
{
   var path=""; var type,existance,shapevalue;
    var data = shapesname.customshapename;
    for(var id in data){
      if(data[id].name=== shape){
        existance=true;
      }
    }
     if(existance){
        $.confirm("ShapeName Is Already Exist Do you want to replace the existing path ? "
        ,  function(response) {
          if(response) {
            for(var i=1; i<svgcontent.lastChild.childNodes.length;i++){
                path+=canvas.convertToPath(svgcontent.lastChild.childNodes[i],null);
                shapevalue =svgcontent.lastChild.childNodes[i];
            }
            type="update";
            addcustomshape(path,shape,type);
          } else {
            $.prompt("Enter Shape Name", "", function(shapename) {
              if (shapename) {
                if(shapename===shape){
                  $.alert("Shape is not saved please try again");
                  return;
                } 
                for(var i=1; i<svgcontent.lastChild.childNodes.length;i++){
                    path+=canvas.convertToPath(svgcontent.lastChild.childNodes[i],null);
              ...
              type="add";
              addcustomshape(path,shape,type,shapevalue);
             });
          } });
      }
      else{
...
this.ungroupSelectedElement = function() {
  ...
  // a path element and coverts every three line segments into a single bezier
  // curve in an attempt to smooth out the free-hand
  var smoothPolylineIntoPath = this.smoothPolylineIntoPath = function(element) {
    var points = element.points;
    var N = points.numberOfItems;
    if (N >= 4) {
